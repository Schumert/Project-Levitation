...
namespace SlimUI.ModernMenu
{
	public class UIMenuManager : MonoBehaviour
	{
		...
		[SerializeField] private SceneField _room1;
		...
		public void LoadScene()
		{
			...
			Initializer.SpawnPersistentObjects();
			_scenesToLoad.Add(SceneManager.LoadSceneAsync(_room1));
		...
		{ // scene name is just the name of the current scene being loaded
			...
			for (int i = 0; i < _scenesToLoad.Count; i++)
			{
				...
				{
					float progress = Mathf.Clamp01(_scenesToLoad[i].progress / .95f);
					loadingBar.value = progress;

					if (_scenesToLoad[i].progress >= 0.9f && waitForInput)
					{
					...
					else if (_scenesToLoad[i].progress >= 0.9f && !waitForInput)
					{
						_scenesToLoad[i].allowSceneActivation = true;
					}
